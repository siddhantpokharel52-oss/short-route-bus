    ...
    onError: (err: any) => {
      ...
      if (errors && typeof errors === 'object') {
        const msg = Object.entries(errors)
          ...
          .join(' | ')
        toast.error(msg || 'Failed to update company info')
      } else {
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        ...
        <div className="card">
          ...
          <form onSubmit={companyForm.handleSubmit((d) => companyMutation.mutate(d))} className="space-y-4">
              ...
              <div className="flex items-center gap-4">
                ...
                >
                  {currentLogoSrc ? (
                    ...
                    <div className="flex h-full flex-col items-center justify-center text-gray-400">
                      ...
                      <span className="mt-1 text-[10px]">Logo</span>
                    </div>
                  )}
                  {/* Hover overlay */}
                ...
                <div className="min-w-0 flex-1">
                  <p className="text-sm text-gray-600">
                    {currentLogoSrc
                      ? 'Logo uploaded. Click the image to change.'
                      : 'Upload your company logo. It will appear in the sidebar.'}
                  </p>
                  <p className="mt-0.5 text-xs text-gray-400">PNG, JPG or SVG · max 2 MB</p>
            ...
            <div className="border-t border-gray-100 pt-4 space-y-4">
              <Input label="Company Name" required {...companyForm.register('company_name')} />
              <Input label="Contact Email" type="email" {...companyForm.register('contact_email')} />
              <Input label="Contact Phone" type="tel" {...companyForm.register('contact_phone')} />
              <Input label="Registration No." {...companyForm.register('registration_no')} />
              <Input label="PAN / Tax Number" {...companyForm.register('tax_pan')} />
              <div>
        ...
        <div className="card space-y-4">
          ...
          <div>
            ...
            <LanguageToggle />
            <p className="mt-1 text-xs text-gray-400">Current: {language === 'en' ? 'English' : 'नेपाली'}</p>
          </div>
          <div>
            <label className="mb-2 block text-sm font-medium text-gray-700">Calendar</label>
            <CalendarToggle />
            <p className="mt-1 text-xs text-gray-400">Current: {calendarType}</p>
          </div>
        </div>
