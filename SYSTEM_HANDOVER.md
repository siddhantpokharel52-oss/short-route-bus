...
**Auth scheme corrected against NamastePay's real docs (2026-09-17)**: this was originally built as a guess (HTTP Basic with `client_id`/`client_secret`) before any real NamastePay documentation existed. The user later supplied NamastePay's actual checkout docs and Swagger UI, which showed a v1→v2 migration notice; fetching the real OpenAPI spec directly (`https://testpay.namastepay.com/api/v2/openapi.json`, then confirmed byte-for-byte against a copy the user pasted directly) confirmed auth is a single API key sent as the `X-API-KEY` header (generated via NamastePay's merchant portal), and that `initiate` takes `amount` (integer paisa, not decimal NPR), `reference_id`, `remarks`, and optional `amount_breakdown` — no `order_id`, `return_url`, or `customer` field exists on their side. `NamastePayConfig.client_id`/`client_secret` were renamed to a single `api_key` field (no real credentials were ever saved under the old ones, so this was a clean rename, not a data migration). Re-verified live: a dummy key now produces a real `403 Forbidden` from the correct `/api/v2/initiate` endpoint (previously got a `401` from what turned out to be the wrong, soon-to-be-deprecated v1 endpoint with the wrong auth scheme entirely).

**Return-URL mechanism now confirmed, not a guess**: the spec's `callbacks` section states the customer is redirected back to `{$your_return_url}` "**that you can set when generating an API key**" — i.e. configured once in the merchant portal, not passed per-request (matches why `initiate`'s request schema has no `return_url` field). The redirect carries `checkout_id`/`reference_id`/`transaction_id`/`status`/`amount` as query params, but per this file's existing note in `accounting/signals.py`, a future purchase flow must still call `enquire_checkout()` server-side before trusting any of it.
